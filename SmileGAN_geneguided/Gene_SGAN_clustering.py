...
def clustering_result(model_dirs, ncluster, imaging_data, gene_data, fraction, covariate=None):
	"""
	...
	if len(all_prediction_labels) == 1:
		return np.array(all_prediction_labels[0]), np.array(all_prediction_probabilities[0]), 1, 0, mean_n_sig_snps, std_n_sig_snps
	else:
...
def cross_validated_clustering(imaging_data, gene_data, ncluster, fold_number, fraction, start_saving_epoch, max_epoch, output_dir, WD_threshold, AQ_threshold, \
	...
	cluster_label, cluster_prob, mean_ari, std_ari, mean_n_sig_snp, std_n_sig_snp = clustering_result(saved_models, ncluster, imaging_data, gene_data, fraction, covariate)
	...
	pt_data["N-Sig-SNPs = %.2f+- %.2f" %(mean_n_sig_snp, std_n_sig_snp)] = ''
